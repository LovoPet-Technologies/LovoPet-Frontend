...
export default function VetFilterSidebar({
  ...
  return (
    <aside className="w-full lg:w-72 shrink-0 rounded-3xl bg-white p-4 sm:p-6 shadow-sm ring-1 ring-black/5 lg:sticky lg:top-6 lg:self-start">
      {/* Header (tap to expand/collapse on mobile only) */}
      <button
        type="button"
        onClick={() => setMobileOpen((prev) => !prev)}
        className="flex w-full items-center justify-between gap-2 text-[#5C2A73] lg:pointer-events-none lg:cursor-default"
      >
      ...
      <div
        className={`${
          mobileOpen ? "mt-6 max-h-[1000px] opacity-100" : "max-h-0 opacity-0"
